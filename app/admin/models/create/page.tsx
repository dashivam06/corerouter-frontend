...
function StepNode({
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      >
        {completed ? <CheckCircle className="size-5" /> : idx}
      </div>
      <div className={`mt-2 text-xs ${active || completed ? "text-zinc-900 font-medium" : "text-zinc-400"}`}>
        {label}
      </div>
    </div>
  );
...
export default function AdminCreateModelPage() {
  ...
  const [provider, setProvider] = useState("Fleebug");
    ...
    <div>
      ...
      <div className="mb-8 flex items-center gap-0">
        <div className="flex w-full items-center gap-6">
          <StepNode
            ...
            label="Identity"
          />
          <div className={`h-px flex-1 ${step > 1 ? "bg-zinc-950" : "bg-zinc-200"}`} />
          <StepNode
            idx={2}
            active={step === 2}
            completed={step > 2}
            label="Documentation"
          />
          <div className={`h-px flex-1 ${step > 2 ? "bg-zinc-950" : "bg-zinc-200"}`} />
          <StepNode
            idx={3}
            active={step === 3}
            completed={step > 3}
            label="Billing"
          />
          <div className={`h-px flex-1 ${step > 3 ? "bg-zinc-950" : "bg-zinc-200"}`} />
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-5">
            ...
            <div className="space-y-5">
              ...
              <label className="block">
                ...
                <datalist id="providers">
                  {["Fleebug", "OpenAI", "Anthropic"].map((p) => (
          ...
          {step === 4 ? (
            <div className="space-y-5">
              <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
                ...
                <div className="rounded-2xl border border-zinc-200 bg-white p-5">
                  <div className="text-sm font-medium text-zinc-900">Doc sections</div>
                  <div className="mt-2 text-xs text-zinc-500">
                    {docsTitle ? `- ${docsTitle}` : "—"}
                  </div>
                </div>
        ...
        <div className="xl:col-span-2">
          <div className="sticky top-6 rounded-2xl border border-zinc-200 bg-white p-4">
            <div className="mb-3 text-xs font-medium uppercase tracking-wide text-zinc-400">
              Preview in catalog
            </div>
            <div className="text-sm text-zinc-400">{provider || "Provider"}</div>
            <div className="mt-1 text-[15px] font-semibold text-zinc-900">
              {fullName || "Model name"}
            </div>
            <div className="mt-1">{username ? usernamePill(username) : null}</div>
          ...
          <div className="mt-5 flex justify-between gap-3">
            ...
            <button
              ...
              className="rounded-xl border border-zinc-200 bg-zinc-950 px-4 py-2 text-sm font-medium text-white hover:bg-zinc-900"
            >
              Next
            </button>
          </div>
